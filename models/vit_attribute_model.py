...
class OnClassify_v3(nn.Module):
    ...
    def __init__(self, args):
        super(OnClassify_v3, self).__init__() #调用父类，确保正确初始化nn.Module类

        self.net = models_vit.__dict__[args.model](
            num_classes=2, #表示神经网络输出类别为4
            drop_path_rate=args.drop_path, #训练过程中使用的 drop path 的比率，一种正则化技术
            global_pool=args.global_pool, #是否使用全局池化层
        )
