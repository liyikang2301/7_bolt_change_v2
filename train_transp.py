...
DATA_INPUT = r"F:\study\3_5programtest\7_bolt_change_v2\data_plastic\org_backdrop_sort\1_plastic_data_set/"  #用于生成训练集与测试集路径
...
def get_args_parser():
    ...
    parser.add_argument('--finetune', default=r'F:\study\3_5programtest\7_bolt_change_v2\model\mae_pretrain_vit_base.pth',
...
def train(model):
    ...
    train_set = TripleDataset(DATA_INPUT + 'attribute_transparency_train.tsv')
    test_set = TripleDataset(DATA_INPUT + 'attribute_transparency_test.tsv')  # use triple
    ...
    for epoch in range(epoch_num): #迭代 epoch_num 次。外层迭代，表示训练过程的轮次。
        ...
        if round(acc / (test_set.len*2), 2) >= best_score:
            ...
            name_str = './test_checkpoint_transp_epoch50/model_transp_best_4neg.pkl'.replace('best', str(best_score))
...
def iii():
    #在这个代码中，推理的目标是输出每个属性的推理结果。
    attribute_out_dic={0:'notrans1',1:'notrans2',2:'trans',3:'notrans_abs',4:'trans_pvc_yello'} #一个字典，将索引映射到相应的属性名称。例如，0 对应 'hex1'，1 对应 'round1'，以此类推。
    # model = torch.load('./checkpoint/reason_model_zero_1.0_4neg.pkl', map_location=torch.device(device))
    model = torch.load(r"F:\study\3_5programtest\7_bolt_change_v2\test_checkpoint_transp_epoch50\model_transp_1.0_4neg.pkl", map_location=torch.device(device))
    #从指定路径加载模型。这里加载的模型是训练过程中表现最好的模型，它用于推理。
    for i in range(len(attribute_out_dic)): #遍历属性字典中的每个属性。
...
def infer_checkpoint(model,atribute_out): #推理的函数
    #一个字典，将属性名称映射到相应的螺栓类型。例如，'hex1' 对应 'out_hex_bolt'，'round1' 对应 'in_hex_bolt'，以此类推。
    attribute_out2bolt={'notrans1':'pe_notrans_noblack','notrans2':'pp_black_notrans_black','trans':'pvc_trans_noblack','notrans_abs':'abs_notrans_noblack','trans_pvc_yello':'pvc_yello_background'}
    attribute_out2index={'notrans1':0,'trans':1,'notrans2':0,'notrans_abs':0,'trans_pvc_yello':0} #这里对应着哪个位置的标签索引，其中，0位置对应标签0，1位置对应标签1
    # 一个字典，将属性名称映射到相应的索引。例如，'hex1' 对应 0，'round1' 对应 1，以此类推。
    op_list = [
            {'op': 'objects', 'param': ''},
            {'op':'filter_nearest_obj', 'param': ''},
    ...
    img_list = list(range(200, 248)) #一个包含待推理的图像ID的列表。在这里，img_list 中只包含一个图像ID。
    # img_list = [1]
    print('[INFO]---------- 评分测试 ---------')
    tol_num=0
    val_num=0
...
if __name__ == "__main__":
    ...
    # Model Load
    # model_transp = Reasoning(args) #初始模型
    # for name, param in model_out.named_parameters(): #named_parameters()函数用于获取模块参数的标准方式，他使得可以方便地访问和操作神经网络模型中的参数。
    #     print(name, param.size(), type(param))
    # model_transp = torch.load('./checkpoint/net_in_0.9862.pkl',map_location=torch.device(device)) #导入之前训练好的模型

    # train(model_transp) # 训练流程
