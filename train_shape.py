...
def train(model):
    ...
    train_set = TripleDataset(DATA_INPUT + 'attribute_shape_train.tsv')
    test_set = TripleDataset(DATA_INPUT + 'attribute_shape_test.tsv')  # use triple
    ...
    for epoch in range(epoch_num): #迭代 epoch_num 次。外层迭代，表示训练过程的轮次。
        ...
        if round(acc / (test_set.len*2), 2) >= best_score:
            ...
            name_str = './test_checkpoint_shape_epoch50/model_shape_best_4neg.pkl'.replace('best', str(best_score))
...
def iii():
    #在这个代码中，推理的目标是输出每个属性的推理结果。
    attribute_out_dic={0:'noellipse1',1:'noellipse2',2:'ellipse',3:'noellipse_abs'} #一个字典，将索引映射到相应的属性名称。例如，0 对应 'hex1'，1 对应 'round1'，以此类推。
    # model = torch.load('./checkpoint/reason_model_zero_1.0_4neg.pkl', map_location=torch.device(device))
    model = torch.load(r"F:\study\3_5programtest\7_bolt_change_v2\test_checkpoint_shape_epoch50\model_shape_1.0_4neg.pkl",map_location=torch.device(device))
    #从指定路径加载模型。这里加载的模型是训练过程中表现最好的模型，它用于推理。
    for i in range(len(attribute_out_dic)): #遍历属性字典中的每个属性。
...
def infer_checkpoint(model,atribute_out): #推理的函数
    #一个字典，将属性名称映射到相应的螺栓类型。例如，'hex1' 对应 'out_hex_bolt'，'round1' 对应 'in_hex_bolt'，以此类推。
    attribute_out2bolt={'noellipse1':'pe_notrans_noblack','noellipse2':'pp_black_notrans_black','ellipse':'pvc_trans_noblack','noellipse_abs':'abs_notrans_noblack'}
    attribute_out2index={'noellipse1':0,'ellipse':1,'noellipse2':0,'noellipse_abs':0} #这里对应着哪个位置的标签索引，其中，0位置对应标签0，1位置对应标签1
    # 一个字典，将属性名称映射到相应的索引。例如，'hex1' 对应 0，'round1' 对应 1，以此类推。
    op_list = [
            {'op': 'objects', 'param': ''},
            {'op':'filter_nearest_obj', 'param': ''},
            {'op':'obj_attibute', 'param': [2,attribute_out2index[atribute_out]]}
    ] #一个包含操作的列表，每个操作是一个字典，指定了操作的类型和参数
    img_list = list(range(231, 248)) #一个包含待推理的图像ID的列表。在这里，img_list 中只包含一个图像ID。
    # img_list = [1]
    print('[INFO]---------- 评分测试 ---------')
    tol_num=0
    val_num=0
            ...
            for i in range(2): #构造测试集的操作列表，由于该属性有两个概念，每个概念要一个操作列表，由于神经网络输出两个概念的值，故每个操作列表的概念矩阵索引分别对应了各自concept的位置
                ...
                q={
                    ...
                    "op_list": [
                        ...
                        {"op": "obj_attibute", "param":[2,i]}],  # 通过filter判定该物体是否是triple中subject物体
...
if __name__ == "__main__":
    ...
    # model_shape = torch.load('./checkpoint/net_in_0.9862.pkl',map_location=torch.device(device)) #导入之前训练好的模型
